...
static void process_packet(struct smi2021 *smi2021, u8 *p, int size)
{
	...
	if (size % 0x400 != 0) {
//		printk_ratelimited(KERN_INFO "smi2021::process_packet: size: %d\n",
//				__func__, size);
		return;
	}
...
static void smi2021_release(struct v4l2_device *v4l2_dev)
{
	...
	qv4l2_ctrl_handler_free(&smi2021->ctrl_handler);
	qv4l2_device_unregister(&smi2021->v4l2_dev);
					...
					const struct usb_device_id *devid)
{
    ...
    desc = &intf->altsetting[2].endpoint[0].desc;
	...
	rc = qv4l2_device_register(dev, &smi2021->v4l2_dev);
	if (rc < 0) {
	...
	smi2021->gm7113c_subdev = qv4l2_i2c_new_subdev_board(&smi2021->v4l2_dev,
							&smi2021->i2c_adap,
							&gm7113c_info, NULL);
...
static void smi2021_usb_disconnect(struct usb_interface *intf)
{
	...
	qvideo_unregister_device(&smi2021->vdev);
	qv4l2_device_disconnect(&smi2021->v4l2_dev);
	...
	qv4l2_device_put(&smi2021->v4l2_dev);
}
...
static int __init smi2021_module_init(void)
{
        int rc;

        rc = usb_register(&smi2021_usb_driver);
        if (rc)
                printk(KERN_ERR "smi2021: usb_register failed rc=%d\n", rc);
        return rc;
}

static void __exit smi2021_module_exit(void)
{
        usb_deregister(&smi2021_usb_driver);
}

module_init(smi2021_module_init);
module_exit(smi2021_module_exit);
