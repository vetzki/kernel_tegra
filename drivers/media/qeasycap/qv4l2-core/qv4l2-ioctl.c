...
#include "qlinux/qvideodev2.h"

#include "qmedia/qv4l2-common.h"
#include "qmedia/qv4l2-ioctl.h"
#include "qmedia/qv4l2-ctrls.h"
#include "qmedia/qv4l2-fh.h"
#include "qmedia/qv4l2-event.h"
#include "qmedia/qv4l2-device.h"
#include "qmedia/qvideobuf2-core.h"
 ...
 */
const char *qv4l2_norm_to_name(v4l2_std_id id)
{
	u32 myid = id;
	int i;
...
EXPORT_SYMBOL(qv4l2_norm_to_name);

/* Returns frame period for the given standard */
void qv4l2_video_std_frame_period(int id, struct v4l2_fract *frameperiod)
{
...
EXPORT_SYMBOL(qv4l2_video_std_frame_period);

/* Fill in the fields of a v4l2_standard structure according to the
   'id' and 'transmission' parameters.  Returns negative on error.  */
int qv4l2_video_std_construct(struct v4l2_standard *vs,
			     int id, const char *name)
{
	vs->id = id;
	qv4l2_video_std_frame_period(id, &vs->frameperiod);
	vs->framelines = (id & V4L2_STD_525_60) ? 525 : 625;
	strlcpy(vs->name, name, sizeof(vs->name));
	return 0;
}
EXPORT_SYMBOL(qv4l2_video_std_construct);

/* ----------------------------------------------------------------- */
/* some arrays for pretty-printing debug messages of enum types      */

const char *qv4l2_field_names[] = {
	[V4L2_FIELD_ANY]        = "any",
...
EXPORT_SYMBOL(qv4l2_field_names);

const char *qv4l2_type_names[] = {
...
EXPORT_SYMBOL(qv4l2_type_names);
...
static void v4l_print_fmtdesc(const void *arg, bool write_only)
{
	...
	pr_cont("index=%u, type=%s, flags=0x%x, pixelformat=%c%c%c%c, description='%.*s'\n",
		p->index, prt_names(p->type, qv4l2_type_names),
...
static void v4l_print_format(const void *arg, bool write_only)
{
	...
	pr_cont("type=%s", prt_names(p->type, qv4l2_type_names));
	switch (p->type) {
	...
	case V4L2_BUF_TYPE_VIDEO_OUTPUT:
		...
		pr_cont(", width=%u, height=%u, "
			...
			prt_names(pix->field, qv4l2_field_names),
	...
	case V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE:
		...
		pr_cont(", width=%u, height=%u, "
			...
			prt_names(mp->field, qv4l2_field_names),
	...
	case V4L2_BUF_TYPE_VIDEO_OUTPUT_OVERLAY:
		...
		pr_cont(", wxh=%dx%d, x,y=%d,%d, field=%s, chromakey=0x%08x, clipcount=%u, clips=%p, bitmap=%p, global_alpha=0x%02x\n",
			...
			prt_names(win->field, qv4l2_field_names),
...
{
	pr_cont("type=%s\n", prt_names(*(u32 *)arg, qv4l2_type_names));
}
...
static void v4l_print_requestbuffers(const void *arg, bool write_only)
{
	...
	pr_cont("count=%d, type=%s, memory=%s\n",
		p->count,
		prt_names(p->type, qv4l2_type_names),
...
static void v4l_print_buffer(const void *arg, bool write_only)
{
	...
	pr_cont("%02ld:%02d:%02d.%08ld index=%d, type=%s, "
		"flags=0x%08x, field=%s, sequence=%d, memory=%s",
			...
			p->index,
			prt_names(p->type, qv4l2_type_names),
			p->flags, prt_names(p->field, qv4l2_field_names),
...
static void v4l_print_exportbuffer(const void *arg, bool write_only)
{
	...
	pr_cont("fd=%d, type=%s, index=%u, plane=%u, flags=0x%08x\n",
		p->fd, prt_names(p->type, qv4l2_type_names),
...
static void v4l_print_event(const void *arg, bool write_only)
{
	...
	case V4L2_EVENT_VSYNC:
		printk(KERN_DEBUG "field=%s\n",
			prt_names(p->u.vsync.field, qv4l2_field_names));
		break;
...
static void v4l_print_sliced_vbi_cap(const void *arg, bool write_only)
{
	...
	pr_cont("type=%s, service_set=0x%08x\n",
			prt_names(p->type, qv4l2_type_names), p->service_set);
	for (i = 0; i < 24; i++)
		printk(KERN_DEBUG "line[%02u]=0x%04x, 0x%04x\n", i,
				p->service_lines[0][i],
				p->service_lines[1][i]);
				...
				struct file *file, void *fh, void *arg)
{
	...
	vfd = qvideo_devdata(file);
	*p = qv4l2_prio_max(&vfd->v4l2_dev->prio);
	return 0;
}
				...
				struct file *file, void *fh, void *arg)
{
	...
	return qv4l2_prio_change(&vfd->v4l2_dev->prio, &vfh->prio, *p);
}
				...
				struct file *file, void *fh, void *arg)
{
	...
	qv4l2_video_std_construct(p, curr_id, descr);
	return 0;
}
				...
				struct file *file, void *fh, void *arg)
{
	...
	if (ret == 0)
		qv4l2_video_std_frame_period(std, &p->parm.capture.timeperframe);
	return ret;
}
				...
				struct file *file, void *fh, void *arg)
{
	...
	if (vfh && vfh->ctrl_handler)
		return qv4l2_queryctrl(vfh->ctrl_handler, p);
	if (vfd->ctrl_handler)
		return qv4l2_queryctrl(vfd->ctrl_handler, p);
				...
				struct file *file, void *fh, void *arg)
{
	...
	if (vfh && vfh->ctrl_handler)
		return qv4l2_querymenu(vfh->ctrl_handler, p);
	if (vfd->ctrl_handler)
		return qv4l2_querymenu(vfd->ctrl_handler, p);
				...
				struct file *file, void *fh, void *arg)
{
	...
	if (vfh && vfh->ctrl_handler)
		return qv4l2_g_ctrl(vfh->ctrl_handler, p);
	if (vfd->ctrl_handler)
		return qv4l2_g_ctrl(vfd->ctrl_handler, p);
				...
				struct file *file, void *fh, void *arg)
{
	...
	if (vfh && vfh->ctrl_handler)
		return qv4l2_s_ctrl(vfh, vfh->ctrl_handler, p);
	if (vfd->ctrl_handler)
		return qv4l2_s_ctrl(NULL, vfd->ctrl_handler, p);
				...
				struct file *file, void *fh, void *arg)
{
	...
	p->error_idx = p->count;
	if (vfh && vfh->ctrl_handler)
		return qv4l2_g_ext_ctrls(vfh->ctrl_handler, p);
	if (vfd->ctrl_handler)
		return qv4l2_g_ext_ctrls(vfd->ctrl_handler, p);
	if (ops->vidioc_g_ext_ctrls == NULL)
		return -ENOTTY;
				...
				struct file *file, void *fh, void *arg)
{
	...
	p->error_idx = p->count;
	if (vfh && vfh->ctrl_handler)
		return qv4l2_s_ext_ctrls(vfh, vfh->ctrl_handler, p);
	if (vfd->ctrl_handler)
		return qv4l2_s_ext_ctrls(NULL, vfd->ctrl_handler, p);
	if (ops->vidioc_s_ext_ctrls == NULL)
		return -ENOTTY;
				...
				struct file *file, void *fh, void *arg)
{
	...
	p->error_idx = p->count;
	if (vfh && vfh->ctrl_handler)
		return qv4l2_try_ext_ctrls(vfh->ctrl_handler, p);
	if (vfd->ctrl_handler)
		return qv4l2_try_ext_ctrls(vfd->ctrl_handler, p);
...
static int v4l_dqevent(const struct v4l2_ioctl_ops *ops,
				struct file *file, void *fh, void *arg)
{
	return qv4l2_event_dequeue(fh, arg, file->f_flags & O_NONBLOCK);
}
...
void qv4l_printk_ioctl(const char *prefix, unsigned int cmd)
{
	const char *dir, *type;
...
EXPORT_SYMBOL(qv4l_printk_ioctl);
		...
		unsigned int cmd, void *arg)
{
	...
	if (v4l2_is_known_ioctl(cmd)) {
		...
		if (use_fh_prio && (info->flags & INFO_FL_PRIO)) {
			ret = qv4l2_prio_check(vfd->prio, vfh->prio);
			if (ret)
				goto done;
		}
	} else {
	...
	if (info->flags & INFO_FL_STD) {
		...
		ret = ops->vidioc_default(file, fh,
			use_fh_prio ? qv4l2_prio_check(vfd->prio, vfh->prio) >= 0 : 0,
			cmd, arg);
	}

done:
	if (debug) {
		qv4l_printk_ioctl(video_device_node_name(vfd), cmd);
		if (ret < 0)
...
long
qvideo_usercopy(struct file *file, unsigned int cmd, unsigned long arg,
	       v4l2_kioctl func)
{
	char	sbuf[128];
	void    *mbuf = NULL;
...
EXPORT_SYMBOL(qvideo_usercopy);

long qvideo_ioctl2(struct file *file,
	       unsigned int cmd, unsigned long arg)
{
	return qvideo_usercopy(file, cmd, arg, __video_do_ioctl);
}
EXPORT_SYMBOL(qvideo_ioctl2);
