...
#include "../qv4l2-core/qmedia/qtveeprom.h"

#include "../saa7115/saa7115.h"

#include "../qv4l2-core/qmedia/qv4l2-clk.h"
...
static void em28xx_card_setup(struct em28xx *dev)
{
	...
	if (dev->board.has_msp34xx)
		qv4l2_i2c_new_subdev(&dev->v4l2_dev, &dev->i2c_adap[dev->def_i2c_bus],
	...
	if (dev->board.tuner_type != TUNER_ABSENT) {
		...
		if (has_demod)
			qv4l2_i2c_new_subdev(&dev->v4l2_dev,
				&dev->i2c_adap[dev->def_i2c_bus], "tuner",
				0, qv4l2_i2c_tuner_addrs(ADDRS_DEMOD));
		if (dev->tuner_addr == 0) {
			enum v4l2_i2c_tuner_type type =
				has_demod ? ADDRS_TV_WITH_DEMOD : ADDRS_TV;
			struct v4l2_subdev *sd;

			sd = qv4l2_i2c_new_subdev(&dev->v4l2_dev,
				&dev->i2c_adap[dev->def_i2c_bus], "tuner",
				0, qv4l2_i2c_tuner_addrs(type));

			if (sd)
				dev->tuner_addr = qv4l2_i2c_subdev_addr(sd);
		} else {
			qv4l2_i2c_new_subdev(&dev->v4l2_dev, &dev->i2c_adap[dev->def_i2c_bus],
...
void em28xx_release_resources(struct em28xx *dev)
{
	...
	qv4l2_ctrl_handler_free(&dev->ctrl_handler);

	qv4l2_device_unregister(&dev->v4l2_dev);
			   ...
			   int minor)
{
	...
	retval = qv4l2_device_register(&interface->dev, &dev->v4l2_dev);
	if (retval < 0) {
		em28xx_errdev("Call to qv4l2_device_register() failed!\n");
		return retval;
	}
	...
	if (dev->audio_mode.ac97 != EM28XX_NO_AC97) {
		qv4l2_ctrl_new_std(hdl, &em28xx_ctrl_ops,
			V4L2_CID_AUDIO_MUTE, 0, 1, 1, 1);
		qv4l2_ctrl_new_std(hdl, &em28xx_ctrl_ops,
			V4L2_CID_AUDIO_VOLUME, 0, 0x1f, 1, 0x1f);
	} else {
		/* install the em28xx notify callback */
		v4l2_ctrl_notify(qv4l2_ctrl_find(hdl, V4L2_CID_AUDIO_MUTE),
				em28xx_ctrl_notify, dev);
		v4l2_ctrl_notify(qv4l2_ctrl_find(hdl, V4L2_CID_AUDIO_VOLUME),
				em28xx_ctrl_notify, dev);
	}
...
static void em28xx_usb_disconnect(struct usb_interface *interface)
{
	...
	qv4l2_device_disconnect(&dev->v4l2_dev);
...
static int __init em28xx_module_init(void)
{
        ...
        rc = usb_register(&em28xx_usb_driver);
        if (rc)
                printk(KERN_ERR "em28xx: usb_register failed rc=%d\n", rc);
        return rc;
}
