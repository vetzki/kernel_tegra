...
#include "../qv4l2-core/qmedia/qv4l2-ioctl.h"
#include "../qv4l2-core/qmedia/qv4l2-event.h"
			...
			struct v4l2_format *f)
{
	...
	if (dev->board.is_em2800) {
		...
		qv4l_bound_align_image(&width, 48, maxw, 1, &height, 32, maxh,
				      1, 0);
	}
			 ...
			 struct v4l2_streamparm *p)
{
	...
	if (dev->board.is_webcam)
		...
		qv4l2_video_std_frame_period(dev->norm,
...
{
	struct video_device *vdev = qvideo_devdata(file);
	struct em28xx_fh      *fh  = priv;
	struct em28xx         *dev = fh->dev;
...
static int em28xx_v4l2_open(struct file *filp)
{
	...
	em28xx_videodbg("open dev=%s type=%s users=%d\n",
			video_device_node_name(vdev), qv4l2_type_names[fh_type],
			dev->users);
...
void em28xx_release_analog_resources(struct em28xx *dev)
{
	...
	if (dev->radio_dev) {
		if (video_is_registered(dev->radio_dev))
			qvideo_unregister_device(dev->radio_dev);
		else
			qvideo_device_release(dev->radio_dev);
		dev->radio_dev = NULL;
	}
	if (dev->vbi_dev) {
		...
		if (video_is_registered(dev->vbi_dev))
			qvideo_unregister_device(dev->vbi_dev);
		else
			qvideo_device_release(dev->vbi_dev);
		dev->vbi_dev = NULL;
	}
	if (dev->vdev) {
		em28xx_info("V4L2 device %s deregistered\n",
		...
		if (video_is_registered(dev->vdev))
			qvideo_unregister_device(dev->vdev);
		else
			qvideo_device_release(dev->vdev);
		dev->vdev = NULL;
	}
}
...
static const struct v4l2_ioctl_ops video_ioctl_ops = {
	...
	.vidioc_unsubscribe_event = qv4l2_event_unsubscribe,
#ifdef CONFIG_VIDEO_ADV_DEBUG
...
static const struct video_device em28xx_video_template = {
	...
	.release                    = qvideo_device_release_empty,
...
/*	em28xx_write_reg(dev, EM28XX_R0E_AUDIOSRC, 0xc0); audio register */
	...
	if (NULL == qv4l2_ctrl_find(&dev->ctrl_handler, V4L2_CID_CONTRAST))
		qv4l2_ctrl_new_std(&dev->ctrl_handler, &em28xx_ctrl_ops,
				  V4L2_CID_CONTRAST,
				  0, 0x1f, 1, CONTRAST_DEFAULT);
	if (NULL == qv4l2_ctrl_find(&dev->ctrl_handler, V4L2_CID_BRIGHTNESS))
		qv4l2_ctrl_new_std(&dev->ctrl_handler, &em28xx_ctrl_ops,
				  V4L2_CID_BRIGHTNESS,
				  -0x80, 0x7f, 1, BRIGHTNESS_DEFAULT);
	if (NULL == qv4l2_ctrl_find(&dev->ctrl_handler, V4L2_CID_SATURATION))
		qv4l2_ctrl_new_std(&dev->ctrl_handler, &em28xx_ctrl_ops,
				  V4L2_CID_SATURATION,
				  0, 0x1f, 1, SATURATION_DEFAULT);
	if (NULL == qv4l2_ctrl_find(&dev->ctrl_handler, V4L2_CID_BLUE_BALANCE))
		qv4l2_ctrl_new_std(&dev->ctrl_handler, &em28xx_ctrl_ops,
				  V4L2_CID_BLUE_BALANCE,
				  -0x30, 0x30, 1, BLUE_BALANCE_DEFAULT);
	if (NULL == qv4l2_ctrl_find(&dev->ctrl_handler, V4L2_CID_RED_BALANCE))
		qv4l2_ctrl_new_std(&dev->ctrl_handler, &em28xx_ctrl_ops,
				  V4L2_CID_RED_BALANCE,
				  -0x30, 0x30, 1, RED_BALANCE_DEFAULT);
	if (NULL == qv4l2_ctrl_find(&dev->ctrl_handler, V4L2_CID_SHARPNESS))
		qv4l2_ctrl_new_std(&dev->ctrl_handler, &em28xx_ctrl_ops,
				  V4L2_CID_SHARPNESS,
	...
	qv4l2_ctrl_handler_setup(&dev->ctrl_handler);
